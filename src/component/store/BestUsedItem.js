...
const styles = StyleSheet.create({
    container: {marginRight: 14.2},
    productImg: {width: 224.8, height: 224.8, borderRadius: 5},
    title: {
...
export const BestUsedItem = (props) => {
    ...
    return (
        <TouchableOpacity
            ...
            {item.sale_mth_cd === CodeText.sale_mth_cd_a ? (
                ...
                <Text style={[styles.price, {marginTop: 2.5}]}>{`${formattedNumber(item.sale_prc)}${StoreText.fromWon}`}</Text>
            )}
        </TouchableOpacity>
    );
};
