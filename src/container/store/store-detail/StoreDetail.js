...
class StoreDetail extends Component {
    ...
    onShare = async () => {
        ...
        const link = `${appScheme.app_scheme}/STORE/goods_no=${goods.goods_no}&sale_mth_cd=${CodeText.sale_mth_cd_i}`
        try {
    ...
    renderProduct = () => {
        const {currentMoreOptionIdx, goods, img_list, activeSlide} = this.state;
        return (
            <View style={{marginHorizontal: 20}}>
                {/*{img_list.length > 0 && (*/}
                {/*    <View style={styles.paginationPagingBox}>*/}
                {/*        <Text style={styles.paging}>*/}
                {/*            <Text style={{ color: "#191919" }}>{activeSlide + 1}</Text>/{img_list.length}*/}
                {/*        </Text>*/}
                {/*    </View>*/}
                {/*)}*/}
                ...
                <View style={styles.latestBidBox}>
                    ...
                    {goods.sale_mth_cd === CodeText.sale_mth_cd_a ? (
                        <Text style={styles.currentPrice}>{formattedNumber(goods.low_or_dpst_price)}</Text>
                    ) : (
                ...
                {/*  <View style={{ alignItems: "center" }}>*/}
                {/*    <Text style={styles.priceTitle} />*/}
                {/*    <Text style={styles.priceDesc}>-</Text>*/}
                {/*  </View>*/}
                {/*</View>*/}
                {this.renderSalesStatus()}
    ...
    renderSalesStatus = () => {
        const {goods, isSale} = this.state;
        const barWidth = 100 / parseInt(goods.sale_cnt || 1);

        return (
            <View style={styles.auctionStatusBox}>
                <Text style={styles.auctionStatus}>{StoreText.instantPurchaseStatus}</Text>
                <View style={{flexDirection: "row"}}>
                    <View style={[styles.pinkRound, {backgroundColor: isSale ? Colors.MAIN_COLOR : "#333333"}]}/>
                    <Text style={styles.proceeding}>{isSale ? StoreText.proceeding : goods.sale}</Text>
                </View>

                <View style={{marginTop: 23, width: "100%"}}>
                    <Text style={styles.salesQuantity}>
                        {StoreText.salesQuantity} {(parseInt(goods.sale_cnt || 0))}개
                    </Text>
                    <View style={styles.auctionProgressBar}>
                        <View
                            style={[
                                styles.auctionActiveProgressBar,
                                {width: `${barWidth * parseInt(goods.sale_cnt_immedi) >= 100 ? 100 : barWidth * parseInt(goods.sale_cnt_immedi)}%`},
                            ]}
                        />
                    </View>
                    {barWidth * parseInt(goods.sale_cnt_immedi) > 60 && barWidth * parseInt(goods.sale_cnt_immedi) < 100 ? (
                        <View style={{marginTop: 2}}>
                            <View
                                style={[
                                    styles.triangle,
                                    {
                                        marginLeft: `${barWidth * parseInt(goods.sale_cnt_immedi) - 2.4}%`,
                            ...
                            <View
                                style={[
                                    ...
                                    {marginLeft: `${barWidth * parseInt(goods.sale_cnt_immedi) - 24}%`},
                                ]}
                            >
                                <Text style={styles.remainingNumber}>
                                    {`${parseInt(goods.sale_cnt || 0) - parseInt(goods.sale_cnt_immedi || 0)}개 ${StoreText.left}`}
                                </Text>
                            </View>
                        </View>
                    ) : barWidth * parseInt(goods.sale_cnt_immedi) >= 100 ? (
                        <View style={{ marginTop: 2 }}>
                            <View
                                style={[
                                    styles.triangle,
                        ...
                        <View style={{marginLeft: `${barWidth * parseInt(goods.sale_cnt_immedi)}%`, marginTop: 2}}>
                            <View
                                style={[
                                    styles.triangle,
                                    {
                ...
                {goods.sale_end_dt && (
                    <View style={styles.deadlineView}>
                        <View>
                            <Text style={styles.regularTitle}>{StoreText.salesClosingDate}</Text>
                            <Text
                                style={styles.auctionDeadline}>{`${dateUtil.formatKCT("koreanTime", goods.sale_end_dt)}`}</Text>
                        </View>
    ...
    renderMoreProductInfoTabs = () => {
        ...
        return (
            <View style={styles.moreProductInfoTabs}>
                ...
                >
                    <Text style={{
                        fontFamily: currentMoreOptionIdx === 1 ? Fonts.AppleSB : Fonts.AppleR,
                        fontSize: 13,
                        color: "#191919"
                    }}>
                        {StoreText.contactUs}
    ...
    renderMoreInfomation = () => {
        ...
        return (
            <View style={{marginTop: 15, marginBottom: 30, width: "100%"}}>
                ...
                <View style={styles.qnaPageContainer}>
                    ...
                    {pagination.map((item) => {
                        return (
                            ...
                            >
                                <Text
                                    style={[styles.qnaPage, item === inqry_page ? {color: "#191919"} : {color: "#aaaaaa"}]}>{item + 1}</Text>
                            </TouchableOpacity>
                        )
                    })}
                    <TouchableOpacity
        ...
        return (
            <View style={styles.submitBtnBox}>
                {isSale && !isSold ? (
                    ...
                    >
                        <Text style={styles.submit}>{StoreText.buyNow}</Text>
                        {/*<Text style={styles.optionalText}>810,000원</Text>*/}
                    </TouchableOpacity>
                ) : (
                    <TouchableOpacity
                        style={[
